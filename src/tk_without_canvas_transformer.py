...
def generate_tk(json_path, output_py):
    ...
    with open(json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    tk_writer.write('from tkinter import *\n', output_py)
    tk_writer.write('root = Tk()\n', output_py)
    ...
    def handle_element(element, parent_name, parent_bbox=None):
        ...
        if etype == 'TEXT' or ename == 'text':
            ...
            tk_writer.write(f'{var} = Label({parent_name}, text="{text}", font=("Arial", {font_size}), fg="{text_color}")\n', output_py)
            tk_writer.write(f'{var}.place(x={x}, y={y}, width={width}, height={height})\n', output_py)
            ...
            tk_writer.write(f'{var} = Button({parent_name}, text="{btn_text}")\n', output_py)
            tk_writer.write(f'{var}.place(x={x}, y={y}, width={width}, height={height})\n', output_py)
        elif ename == 'entry' or etype == 'ENTRY':
            var = get_var('entry')
            tk_writer.write(f'{var} = Entry({parent_name})\n', output_py)
            tk_writer.write(f'{var}.place(x={x}, y={y}, width={width}, height={height})\n', output_py)
        elif etype == 'FRAME' or ename == 'frame':
            var = get_var('frame')
            tk_writer.write(f'{var} = Frame({parent_name}, bg="{color_to_hex(element.get("backgroundColor", {"r":1,"g":1,"b":1}))}")\n', output_py)
            tk_writer.write(f'{var}.place(x={x}, y={y}, width={width}, height={height})\n', output_py)
            for child in element.get('children', []):
