...
def transform_json_to_swing(data, output_path, file_id_figma, token_access):
    ...
    swinger_writer.write(f'        frame.getContentPane().setBackground({color_from_figma(bg_color)});\n', file_path)
    ...
    def handle_element(element, parent_name):
        ...
        if etype == "TEXT" or ename == "text":
            ...
            swinger_writer.write(f'        {var}.setForeground({color_from_figma(text_color)});\n', file_path)
            ...
            if button_shape:
                ...
                swinger_writer.write(f'        {var}.setBackground({color_from_figma(fill_color)});\n', file_path)
                swinger_writer.write(f'        {parent_name}.add({var});\n', file_path)
        elif ename == "entry":
            var = get_var('entry')
            ...
            swinger_writer.write(f'        {var}.setBackground({color_from_figma(text_color)});\n', file_path)
            swinger_writer.write(f'        {parent_name}.add({var});\n', file_path)
        elif ename == "image":
            node_id = element["id"]
